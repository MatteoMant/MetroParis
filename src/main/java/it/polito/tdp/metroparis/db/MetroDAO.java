...
public class MetroDAO {
	...
	/**
	 * metodo che mi dice se esiste ALMENO una connessione tra partenza ed arrivo
	 * 
	 * @param partenza
	 * @param arrivo
	 * @return
	...
	public boolean isFermateConnesse(Fermata partenza, Fermata arrivo) {
		...
		try {
			...
			rs.first(); // avrò sicuramente solo una riga perchè ho un COUNT(*)
	...
	public List<Fermata> getFermateConnesse(Fermata partenza){
		...
		try {
			...
			while (rs.next()) { // stiamo ricostruendo degli oggetti Fermata che avevamo già
				Fermata f = new Fermata(rs.getInt("id_Fermata"), rs.getString("nome"),
						new LatLng(rs.getDouble("coordx"), rs.getDouble("coordy")));
				fermate.add(f);
			}
